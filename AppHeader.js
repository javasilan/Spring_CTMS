...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchApprovedRequests = async () => {
      try {
        const response = await fetch('http://localhost:8090/student/route-change-requests');
        if (response.ok) {
          const approvedRequests = await response.json();
          const newApprovedRequests = approvedRequests.filter(request => request.approved);

          if (newApprovedRequests.length > 0) {
            const latestApprovedRequest = newApprovedRequests[newApprovedRequests.length - 1];

            const newApprovedMessage = {
              id: latestApprovedRequest.id,
              message: `Request Approved: ${latestApprovedRequest.requestText}`,
        ...
        <nav className="navbar navbar-expand-lg navbar-dark bg-white">
          <div className="container">
            ...
            <div className="d-flex align-items-center">
              ...
              <Link to={'/login'} className="navbar-brand mx-5 btn btn-danger">
                Logout
              </Link>
            </div>
          </div>
